"""APEX PREDATOR  //  strategies.apex_policy.

The six AI-optimized Apex Predator strategies, composed from the pure
primitives in :mod:`apex_predator.strategies.smc_primitives`. Each
...
from apex_predator.strategies.models import (
    Bar,
    Side,
    StrategyId,
    StrategySignal,
)
from apex_predator.strategies.regime_exclusion import is_regime_excluded
from apex_predator.strategies.smc_primitives import (
...
class StrategyContext:
    ...
    :class:`apex_predator.brain.regime.RegimeType` -- kept as a string so
    this module doesn't import pydantic or the brain package.

    ``confluence_score`` is the 0..10 output of the existing
    :mod:`apex_predator.core.confluence_scorer`; strategies blend their
...
def _risk_mult(ctx: StrategyContext, base_mult: float) -> float:
    ...
    Hard exclusions consult :mod:`apex_predator.strategies.regime_exclusion`
...
def regime_adaptive_allocation(
    ...
    :mod:`apex_predator.strategies.regime_allocator` (portfolio-level) and
    :mod:`apex_predator.funnel.waterfall` (layer-level). This function
...
def rl_full_automation(
    ...
    :mod:`apex_predator.strategies.rl_policy`. This function produces a
