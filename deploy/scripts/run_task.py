...
def _task_kaizen_retro(state_dir: Path) -> dict:
    ...
    from apex_predator.brain.jarvis_v3.kaizen import (
        CycleKind,
        KaizenLedger,
        close_cycle,
    )
...
def _task_shadow_tick(state_dir: Path) -> dict:
    ...
    from apex_predator.brain.jarvis_v3.next_level.shadow import ShadowLedger
...
def _task_strategy_mine(state_dir: Path) -> dict:
    ...
    from apex_predator.brain.jarvis_v3.next_level import strategy_synthesis
    from apex_predator.brain.jarvis_v3.precedent import PrecedentGraph
...
def _task_causal_review(state_dir: Path) -> dict:
    ...
    from apex_predator.brain.jarvis_v3.next_level.causal import CausalDAG
    dag = CausalDAG()
...
def _task_twin_verdict(state_dir: Path) -> dict:
    ...
    from apex_predator.brain.jarvis_v3.next_level.digital_twin import (
        TwinComparator,
    )
    cmp_ = TwinComparator()
    v = cmp_.verdict()
...
def _task_doctrine_review(state_dir: Path) -> dict:
    ...
    from apex_predator.brain.jarvis_v3.philosophy import summarize_doctrine
...
def _task_meta_upgrade(state_dir: Path) -> dict:
    ...
    repo_dir = Path(os.environ.get("APEX_REPO_DIR", r"C:\apex_predator"))
...
def _task_self_test(state_dir: Path) -> dict:
    ...
    try:
        req = urllib.request.Request(
            "https://jarvis.apexpredator.live/health",
...
def _task_audit_summarize(state_dir: Path) -> dict:
    ...
    from apex_predator.brain.jarvis_v3 import nl_query
