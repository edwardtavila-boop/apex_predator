...
from apex_predator.core.confluence_scorer import score_confluence
...
@dataclass
class _Open:
    entry_bar: BarData; side: str; qty: float; entry_price: float
    stop: float; target: float; risk_usd: float; confluence: float
    leverage: float; peak_adverse: float = 0.0
...
class BacktestEngine:
    ...
    def run(self, bars: Iterable[BarData]) -> BacktestResult:
        ...
        for bar in bars:
            ...
            if open_t is not None:
                ...
                if closed is not None:
                    ...
                    curve.append(equity); trades.append(closed); open_t = None
            ...
            if open_t is not None:
                adv = (open_t.entry_price - bar.low) * open_t.qty if open_t.side == "BUY" \
        ...
        if open_t is not None and hist:
            ...
            curve.append(equity); trades.append(final)
    ...
    def _enter(self, bar: BarData, hist: list[BarData], equity: float) -> _Open | None:
        ...
        return _Open(entry_bar=bar, side=side, qty=qty, entry_price=bar.close,
                     stop=stop, target=target, risk_usd=risk_usd,
                     confluence=score.total_score, leverage=float(score.recommended_leverage))
    ...
    def _close(self, t: _Open, bar: BarData, exit_price: float) -> Trade:
        ...
        return Trade(
            entry_time=t.entry_bar.timestamp, exit_time=bar.timestamp,
            symbol=t.entry_bar.symbol, side=t.side,  # type: ignore[arg-type]
            qty=t.qty, entry_price=t.entry_price, exit_price=exit_price,
            pnl_r=round(pnl_r, 4), pnl_usd=round(pnl_usd, 2),
            confluence_score=round(t.confluence, 2), leverage_used=t.leverage,
    ...
    def _finalize(self, trades: list[Trade], curve: list[float]) -> BacktestResult:
        ...
        rets = [(c - p) / p for p, c in zip(curve, curve[1:]) if p > 0.0]
        return BacktestResult(
            strategy_id=self.strategy_id, n_trades=n,
            ...
            expectancy_r=compute_expectancy(trades), profit_factor=compute_profit_factor(trades),
            sharpe=compute_sharpe(rets), sortino=compute_sortino(rets),
