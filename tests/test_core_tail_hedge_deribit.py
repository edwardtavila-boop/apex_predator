...
from apex_predator.core.tail_hedge import price_otm_put_btc_deribit


class TestPriceOtmPutBtcDeribit:
    def test_returns_positive_premium(self):
        q = price_otm_put_btc_deribit(
            btc_spot=60_000.0, otm_pct=15.0, days_to_expiry=30, implied_vol=0.65
        )
    ...
    def test_longer_expiry_means_higher_premium(self):
        short = price_otm_put_btc_deribit(
            btc_spot=60_000.0, days_to_expiry=7, implied_vol=0.65
        )
        long = price_otm_put_btc_deribit(
            btc_spot=60_000.0, days_to_expiry=90, implied_vol=0.65
        )
