"""
APEX PREDATOR  //  scripts.schedule_weekly_review
=================================================
...
Usage
-----
  python -m apex_predator.scripts.schedule_weekly_review           # run now
  python -m apex_predator.scripts.schedule_weekly_review --emit-cron
  python -m apex_predator.scripts.schedule_weekly_review --emit-task-scheduler
...
def _run_weekly_review(tier: str, force_engage: bool) -> int:
    cmd = [
        sys.executable, "-m", "apex_predator.scripts.weekly_review",
        "--tier", tier, "--auto",
    ]
    if not force_engage:
...
def emit_cron() -> str:
    return (
        f"# APEX PREDATOR weekly review — decision #14\n"
        f"# Sunday 20:00 America/New_York\n"
        f"{CRON_LINE} cd {REPO_ROOT} && "
        f"/usr/bin/env python -m apex_predator.scripts.schedule_weekly_review "
        f">> apex_predator/logs/weekly_review_cron.log 2>&1\n"
    )
...
def emit_task_scheduler() -> str:
    exe = sys.executable
    return (
        f"# APEX PREDATOR weekly review — Windows Task Scheduler\n"
        f"schtasks /Create /SC WEEKLY /D SUN /ST 20:00 /TN {TASK_NAME} "
        f"/TR \"\\\"{exe}\\\" -m apex_predator.scripts.schedule_weekly_review\" "
        f"/RL HIGHEST /F\n"
    )
...
def main() -> int:
    ...
    print("APEX PREDATOR -- schedule_weekly_review")
    print("=" * 64)
