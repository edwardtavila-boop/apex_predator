...
from apex_predator.scripts._chaos_drill_matrix import (
    SURFACES,
    SafetySurface,
    coverage_report,
    main,
...
class TestSafetySurface:
    def test_has_drill_true_when_id_set(self):
        s = SafetySurface(surface="x", module_path="apex_predator.x", drill_id="d")
        assert s.has_drill is True

    def test_has_drill_false_when_none(self):
        s = SafetySurface(surface="x", module_path="apex_predator.x", drill_id=None)
...
class TestRenderMarkdown:
    def test_includes_table_header(self):
        ...
        assert "# APEX PREDATOR // Chaos Drill Coverage Matrix" in md
