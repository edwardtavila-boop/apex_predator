# MNQ Engine ↔ Apex Predator — Integration Bridge

**Date:** 2026-04-16
**Purpose:** Single source of truth linking the mnq_backtest / the_firm /
mnq_bot work (Claude Code workspace) to the Apex Predator roadmap
(Claude Cowork + Claude Code). Both sides read from this file.

---

## 1. What the MNQ engine provides to Apex Predator

The MNQ futures bot is the **P0-P1 Engine tier** of the Apex Predator
...
| P1_BRAIN — indicator suite | apex_v3 Pine (ORB, EMA9/21, ATR, ADX, VWAP, Sweep+Reclaim, BB) | `Base/MNQ_Apex_v3_Firm.pine`, `mnq_backtest/pine/` |
| P1_BRAIN — confluence scorer | 15-voice scoring engine (V1-V15) | `Firm/firm_v3/scorer/` (apex_v3_framework), `the_firm_complete/` |
...
- **P1_BRAIN: 15% done.** Indicator suite (apex_v3 Pine) is
...
- **P3_PROOF: 0% in Apex Predator, but MNQ engine has much of this.**
  Walk-forward, Monte Carlo, Deflated Sharpe, regime validation already
  in `mnq_backtest/`. They need to be exposed to the Apex Predator
...
### MNQ engine → Apex Predator (upstream)
...
### Apex Predator → MNQ engine (downstream)
The Apex Predator side supplies:

1. **Baseline/sweep policy** — $5,500 MNQ baseline, 10% above-baseline
   sweep trigger, 60/30/10 stake/seed/reserve split
   - Source: `apex_predator/config.json` → `funnel.sweep_policy`
...
2. **Confluence rubric** — 0-10 scoring weights per voice group
   - Source: `apex_predator/config.json` → `confluence_rubric`
   - Consumer: `the_firm_complete/firm/agents/scorer/` (V3 scorer)
3. **Session/news blackout calendar** — FOMC/CPI/PCE/NFP/GDP windows
   - Source: `apex_predator/brain/calendar/news_blackouts.yaml` (to be built)
...
### Track B — spin up Apex Predator P1_BRAIN via reuse
1. Export the V3 15-voice scorer from `the_firm_complete/` into an
   Apex-compatible package at `apex_predator/brain/scorer/`.
2. Wire `apex_predator/config.json.confluence_rubric` as the *only*
   source of weights — both repos read from the same file.
3. Port the session classifier (`mnq_backtest/data/contract.py`) into
   `apex_predator/brain/sessions/`.
...
- **No Apex Predator phase advances past P3 without a surviving MNQ
  candidate** (C002-B or successor). The engine MUST produce real
  positive expectancy before NQ/crypto/perp bots spin up.
- **Both repos read shared config from `apex_predator/config.json`** —
...
Applied to `apex_predator/roadmap_state.json`:
...
- **Every Apex Predator roadmap tick** updates `apex_predator/roadmap_state.json`.
