...
Context
-------
v0.1.31 shipped the six AI-Optimized Apex Predator strategies as pure
functions in ``apex_predator.strategies.*``. v0.1.32 shipped the
  ...
  * ``strategies/engine_adapter.py`` (~260 lines, new)
    ...
    Public API:
      ...
      - ``context_from_dict(dict, *, kill_switch_active, session_allows_entries,
        overrides)`` -- builds ``apex_policy.StrategyContext`` from a bar
    ...
    Design guarantees:
      ...
      - kill-switch flows through ``context_from_dict`` into the
        ...
        ``apex_policy._risk_mult`` zeroes regardless of detector output.
...
def main() -> None:
    ...
    sa["apex_predator_v0_1_33_engine_adapter"] = {
        "timestamp_utc": now,
        "version": VERSION,
        "bundle_name": (
            "ENGINE ADAPTER -- policy_router wired into the live bot path "
        ...
        "public_api": {
            ...
            "context_from_dict": (
                "Build `strategies.apex_policy.StrategyContext` from a bar "
