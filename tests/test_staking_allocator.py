"""Tests for ``apex_predator.staking.allocator``.
...
def test_import_smoke() -> None:
    ...
    importlib.import_module("apex_predator.staking.allocator")


def test_allocation_config_smoke() -> None:
    """``AllocationConfig`` instantiates with no args (or skips if it requires args)."""
    from apex_predator.staking.allocator import AllocationConfig
    try:
...
def test_allocate_smoke() -> None:
    ...
    from apex_predator.staking.allocator import allocate
...
async def test_rebalance_smoke() -> None:
    ...
    from apex_predator.staking.allocator import rebalance
