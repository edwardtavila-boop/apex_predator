"""
APEX PREDATOR  //  scripts.weekly_review
========================================
...
Usage
-----
    python -m apex_predator.scripts.weekly_review \
        --spec apex_predator/docs/firm_spec_paper_results_v2.json \
        --tier A

    python -m apex_predator.scripts.weekly_review --auto
...
from apex_predator.core.principles_checklist import (  # noqa: E402
    DEFAULT_PRINCIPLES,
    ChecklistAnswer,
    ChecklistReport,
    build_report,
...
def _engage_firm(spec_path: Path) -> dict:
    ...
    try:
        out = subprocess.run(
            [sys.executable, "-m", "apex_predator.scripts.engage_firm_board",
             "--spec", str(spec_path)],
...
def _write_checklist_report(report: ChecklistReport, out_dir: Path) -> Path:
    ...
    lines = [
        "APEX PREDATOR -- Weekly Principles Checklist",
        "=" * 80,
...
def _write(
    ...
    lines: list[str] = []
    lines.append("APEX PREDATOR -- Weekly Firm Review")
    lines.append("=" * 80)
