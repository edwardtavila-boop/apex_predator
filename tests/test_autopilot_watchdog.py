...
from apex_predator.obs.autopilot_watchdog import (
    AutopilotMode,
    AutopilotWatchdog,
    PositionState,
    WatchdogAlertLevel,
...
from apex_predator.obs.decision_journal import Actor, DecisionJournal
...
def test_force_flatten_marked_executed(tmp_path: Path) -> None:
    ...
    wd.register_position(_pos())
    wd.check_all()
    from apex_predator.obs.decision_journal import Outcome
    events = journal.read_all()
...
def test_request_flatten_approval_rejects_non_flatten_alert() -> None:
    ...
    from apex_predator.brain.jarvis_admin import JarvisAdmin  # noqa: PLC0415
    admin = JarvisAdmin()
    wd = AutopilotWatchdog(
...
def test_request_flatten_approval_end_to_end(tmp_path: Path) -> None:
    ...
    from apex_predator.brain.jarvis_admin import (  # noqa: PLC0415
        JarvisAdmin,
        SubsystemId,
        Verdict,
    )
    from apex_predator.brain.jarvis_context import (  # noqa: PLC0415
        EquitySnapshot,
