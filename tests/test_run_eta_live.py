...
from eta_engine.core.kill_switch_latch import KillSwitchLatch, LatchState
...
async def test_apply_verdict_flatten_bot_scoped_by_name():
    ...
    v = KillVerdict(action=KillAction.FLATTEN_BOT, severity=KillSeverity.WARN, reason="test", scope="bot:mnq")
...
async def test_apply_verdict_flatten_bot_scoped_by_symbol():
    ...
    v = KillVerdict(action=KillAction.FLATTEN_BOT, severity=KillSeverity.WARN, reason="funding", scope="bot:ETHUSDT")
...
async def test_apply_verdict_flatten_all_flags_all_bots():
    ...
    mnq = (bindings[0], FakeBot("mnq", "MNQ", "A"))
    eth = (bindings[3], FakeBot("eth_perp", "ETHUSDT", "B"))
    v = KillVerdict(action=KillAction.FLATTEN_ALL, severity=KillSeverity.CRITICAL, reason="port DD", scope="global")
...
async def test_apply_verdict_flatten_tier_b_only():
    ...
    mnq = (bindings[0], FakeBot("mnq", "MNQ", "A"))
    eth = (bindings[3], FakeBot("eth_perp", "ETHUSDT", "B"))
    v = KillVerdict(action=KillAction.FLATTEN_TIER_B, severity=KillSeverity.WARN, reason="corr", scope="tier_b")
    await mod.apply_verdict(v, [mnq, eth], router, disp)
...
async def test_apply_verdict_pause_new_entries():
    ...
    bindings = _fake_bindings()
    mnq = (bindings[0], FakeBot("mnq", "MNQ", "A"))
    v = KillVerdict(action=KillAction.PAUSE_NEW_ENTRIES, severity=KillSeverity.WARN, reason="warn", scope="bot:mnq")
    await mod.apply_verdict(v, [mnq], router, disp)
    assert mnq[1].state.is_paused is True
...
async def test_runtime_flatten_all_stops_loop(tmp_path):
    ...
    runtime = mod.ApexRuntime(cfg, bindings=bindings, kill_switch=mod.KillSwitch(cfg.kill_switch))
    rc = await runtime.run()
    assert rc == 0
    lines = (tmp_path / "runtime.jsonl").read_text(encoding="utf-8").strip().splitlines()
...
class TestLoadRuntimeConfigTickCadence:
    ...
    def test_live_mode_slow_tick_raises(self, tmp_path: Path):
        ...
        from eta_engine.core.kill_switch_runtime import ApexTickCadenceError
...
def test_tradovate_creds_absent_by_default(monkeypatch):
    for k in ("TRADOVATE_CLIENT_ID", "TRADOVATE_CLIENT_SECRET", "TRADOVATE_USERNAME", "TRADOVATE_PASSWORD"):
...
class TestBrokerEquityReconcilerIntegration:
    ...
    async def test_reconciler_alert_fires_once_on_drift_transition(
        ...
        drift_events = [payload for (event, payload) in disp.sent if event == "broker_equity_drift"]
    ...
    async def test_reconciler_no_broker_data_is_logged_not_alerted(
        ...
        assert not any(event == "broker_equity_drift" for (event, _) in disp.sent)
...
class TestBuildBrokerEquityAdapterLiveModeGate:
    ...
    def test_paper_mode_returns_null_adapter(self) -> None:
        from eta_engine.scripts.run_eta_live import (
...
class TestSustainedDriftReAlert:
    ...
    async def test_sustained_drift_silent_within_interval(self, tmp_path):
        ...
        drift_events = [(event, payload) for (event, payload) in disp.sent if event == "broker_equity_drift"]
...
class TestRuntimeLogRotatorWiring:
    ...
    async def test_rotation_emits_log_rotation_event(self, tmp_path):
        ...
        if cfg.log_path.exists():
            ...
            kinds = [json.loads(ln).get("kind") for ln in lines if ln.strip()]
...
class TestTierAInvariantWiring:
    ...
    async def test_oversize_aggregate_fires_alert_once(self, tmp_path):
        ...
        from eta_engine.scripts.run_eta_live import BotBinding
        ...
        violations = [payload for (event, payload) in disp.sent if event == "tier_a_invariant_violation"]
    ...
    async def test_normal_aggregate_does_not_fire_alert(self, tmp_path):
        ...
        violations = [event for (event, _) in disp.sent if event == "tier_a_invariant_violation"]
