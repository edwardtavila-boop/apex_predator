...
from eta_engine.features.crowd_pain_index import (
    CrowdPainIndexFeature,
    compute_cpi,
    cpi_signals_trade,
)
from eta_engine.features.liquidation_map import (
    LiquidationCluster,
    LiquidationHeatmap,
)
...
def test_cpi_trade_gate_requires_three_components_above_07() -> None:
    """Build synthetic context so at least 3 of 5 components >= 0.7 AND score >= 70."""
    import random
    ...
    # Taker ratios need variance (not flat) so stdev > 0 and z-score is defined.
    # Prior mean ~0.5, stdev ~0.03, latest 0.85 → z ≈ 11, strength 1.0.
    taker_ratios = [0.5 + rng.gauss(0.0, 0.03) for _ in range(200)]
    taker_ratios.append(0.85)
    ctx = {
