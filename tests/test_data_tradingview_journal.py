...
def test_record_bar_appends_gzipped_line(tmp_path: Path) -> None:
    ...
    j.record_bar(BarEntry(
        ts=1_714_000_000.0, symbol="BINANCE:BTCUSDT", interval="1",
        o=50_000.0, h=50_100.0, l=49_900.0, c=50_050.0, v=12.5,
    ))
...
def test_record_bar_two_appends_into_same_day_file(tmp_path: Path) -> None:
    j = TradingViewJournal(tmp_path)
    e = BarEntry(
        ts=1_714_000_000.0, symbol="A:B", interval="1",
...
def test_record_indicator_appends_jsonl(tmp_path: Path) -> None:
    j = TradingViewJournal(tmp_path)
    j.record_indicator(IndicatorEntry(
        ts=now_iso(), symbol="X:Y", interval="5",
        indicator="RSI", params="14, close", value=62.4, all=[62.4],
    ))
    j.record_indicator(IndicatorEntry(
        ts=now_iso(), symbol="X:Y", interval="5",
        indicator="MACD", params="12, 26, 9", value=0.4, all=[0.4, 0.3, 0.1],
    ))
...
def test_record_alert_appends_jsonl(tmp_path: Path) -> None:
    j = TradingViewJournal(tmp_path)
    j.record_alert(AlertEntry(
        ts=now_iso(), kind="definition",
        symbol="BINANCE:BTCUSDT", name="BTC > 60k",
        condition=">", value=60_000.0, active=True, fired_at=None,
    ))
...
def test_journal_swallows_oserror_on_bar(tmp_path: Path, monkeypatch) -> None:  # noqa: ANN001
    ...
    j.record_bar(BarEntry(
        ts=1.0, symbol="X:Y", interval="1",
        o=1.0, h=1.0, l=1.0, c=1.0, v=0.0,
    ))
