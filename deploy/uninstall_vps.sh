#!/usr/bin/env bash
# ============================================================================
# APEX PREDATOR // uninstall_vps.sh
# ----------------------------------------------------------------------------
...
for unit in apex-dashboard avengers-fleet jarvis-live; do
...
# 3. Strip crontab entries tagged apex-predator:avengers
log "stripping crontab entries"
EXISTING="$(crontab -l 2>/dev/null || true)"
if [[ -n "$EXISTING" ]]; then
  printf '%s\n' "$EXISTING" | grep -v 'apex-predator:avengers' | crontab -
fi

# 4. Purge state + logs if requested
if [[ "$PURGE" == "1" ]]; then
  log "PURGE: removing state + logs"
  rm -rf "$HOME/.local/state/apex_predator"
  rm -rf "$HOME/.local/log/apex_predator"
fi
