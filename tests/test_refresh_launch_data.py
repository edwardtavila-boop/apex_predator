...
def test_build_plan_refreshes_launch_data_then_republishes_and_verifies() -> None:
    ...
    assert "eta_engine.scripts.fetch_index_futures_bars --symbol NQ --timeframe 5m" in commands[1]
    assert "eta_engine.scripts.extend_nq_daily_yahoo" in commands[2]
    assert "eta_engine.scripts.announce_data_library" in commands[3]
    assert "eta_engine.scripts.paper_live_launch_check --json" in commands[4]


def test_build_plan_can_skip_inventory_and_verify() -> None:
    plan = mod.build_plan(skip_inventory=True, skip_verify=True)

    assert [name for name, _ in plan] == ["mnq_5m", "nq_5m", "nq_daily"]
...
def test_run_plan_reports_success(monkeypatch) -> None:
    ...
    assert [step["name"] for step in summary["steps"]] == ["mnq_5m", "nq_5m", "nq_daily"]
