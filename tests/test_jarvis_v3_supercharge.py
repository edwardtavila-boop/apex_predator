...
class TestVPS:
    ...
    def _snap(self, cpu=30, mem=40, disk=50):
        return vps.VPSSnapshot(
            ...
            load_1m=0.5, load_5m=0.7, load_15m=0.9,
        )
    ...
    def test_failed_service_red(self):
        ...
        assert any(a.action == vps.VPSActionType.RESTART
                   for a in rep.proposed_actions)
    ...
    def test_high_disk_proposes_prune(self):
        ...
        assert any(a.action == vps.VPSActionType.DISK_PRUNE
                   for a in rep.proposed_actions)

    def test_action_to_shell(self):
        req = vps.VPSActionRequest(
            action=vps.VPSActionType.RESTART, service="mnq-bot.service",
            rationale="test",
        )
...
class TestUnleashed:
    ...
    def _ctx(self):
        ...
        return JarvisContext(
            ...
            equity=EquitySnapshot(
                account_equity=50000, daily_pnl=0.0,
                daily_drawdown_pct=0.01, open_positions=1, open_risk_r=1.0,
            ),
            regime=RegimeSnapshot(regime="NEUTRAL", confidence=0.7),
            journal=JournalSnapshot(),
            suggestion=JarvisSuggestion(
                action=ActionSuggestion.TRADE, reason="all gates green",
                confidence=0.8,
            ),
            stress_score=stress,
    ...
    def test_portfolio_breach_downgrades(self):
        ...
        ctx = self._ctx()
        req = ActionRequest(
            subsystem=SubsystemId.BOT_BTC_PERP, action=ActionType.ORDER_PLACE,
            rationale="long btc",
        )
        from apex_predator.brain.jarvis_v3.portfolio import Exposure
        exps = [
