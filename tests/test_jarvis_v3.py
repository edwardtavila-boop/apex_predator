...
from apex_predator.brain.jarvis_v3 import (
    alerts_explain,
    anomaly,
    bandit,
    budget,
...
from apex_predator.brain.model_policy import ModelTier, TaskCategory
...
class TestRegimeStress:
    ...
    def test_reweight_crisis_dominates_macro(self):
        raws = {
            "macro_event": 1.0, "equity_dd": 0.5, "open_risk": 0.5,
            "regime_risk": 0.3, "override_rate": 0.2, "autopilot": 0.0,
            "correlations": 0.0, "macro_bias": 0.5,
        }
...
class TestHorizons:
    ...
    def test_all_horizons_present(self):
        ctx = horizons.project(base_composite=0.3, base_binding="equity_dd",
    ...
    def test_event_bumps_near_horizon(self):
        ctx = horizons.project(
            base_composite=0.2, base_binding="equity_dd",
            hours_until_event=0.25, event_label="FOMC",
        )
        # 15-minute horizon straddles the event -> bump
        assert ctx.next_15m.composite > ctx.now.composite or \
               ctx.next_15m.composite >= 0.4

    def test_overnight_floor(self):
        ctx = horizons.project(
            base_composite=0.05, base_binding="none",
            hours_until_event=None, is_overnight_now=True,
        )
        assert ctx.overnight.composite >= 0.40

    def test_binding_horizon_is_max(self):
        ctx = horizons.project(
            base_composite=0.2, base_binding="x",
            hours_until_event=0.1, event_label="FOMC",
        )
...
class TestCalibration:
    ...
    def test_default_sigmoid_in_01(self):
        feats = calibration.VerdictFeatures(
            verdict="APPROVED", stress_composite=0.3, sizing_mult=1.0,
    ...
    def test_stress_lowers_p_correct(self):
        low = calibration.VerdictFeatures(
            verdict="APPROVED", stress_composite=0.1, sizing_mult=1.0,
        )
        high = calibration.VerdictFeatures(
            verdict="APPROVED", stress_composite=0.9, sizing_mult=1.0,
        )
...
class TestPortfolio:
    ...
    def test_correlated_triggers_breach(self):
        ...
        corr = {
            ("BTC", "ETH"): 0.85, ("BTC", "SOL"): 0.80, ("ETH", "SOL"): 0.82,
        }
...
class TestPreferences:
    ...
    def test_loosen_signal_captured(self):
        ...
        for i in range(5):
            lrn.observe(preferences.OverrideEvent(
                ts=now - timedelta(days=i),
                subsystem="bot.mnq", action="ORDER_PLACE",
                reason_code="overnight_refused", direction="loosen",
            ))
    ...
    def test_tighten_signal_captured(self):
        ...
        for i in range(3):
            lrn.observe(preferences.OverrideEvent(
                ...
                subsystem="bot.mnq", action="ORDER_PLACE",
                reason_code="trade_ok", direction="tighten",
            ))
    ...
    def test_persistence_roundtrip(self, tmp_path: Path):
        ...
        lrn.observe(preferences.OverrideEvent(
            ts=datetime.now(UTC),
            subsystem="s", action="a", reason_code="r", direction="loosen",
        ))
...
class TestCritique:
    ...
    def test_high_fp_gives_red(self):
        ...
        for i in range(20):
            decisions.append(critique.DecisionRecord(
                ts=now - timedelta(hours=i),
                verdict="APPROVED", reason_code="trade_ok",
    ...
    def test_drift_detected(self):
        ...
        for i in range(20):
            ...
            decisions.append(critique.DecisionRecord(
                ...
                verdict="APPROVED", reason_code="x",
                stress_composite=comp,
                outcome_correct=1, realized_r=1.0,
            ))
        rep = critique.critique_window(decisions)
...
class TestPrecedent:
    ...
    def test_record_and_query(self):
        ...
        k = precedent.PrecedentKey(regime="RISK_ON", session_phase="MORNING")
        for _i in range(4):
            g.record(k, precedent.PrecedentEntry(
                ts=datetime.now(UTC), action="TRADE", verdict="APPROVED",
                outcome_correct=1, realized_r=1.0,
            ))
        q = g.query(k)
        assert q.n == 4
    ...
    def test_save_load(self, tmp_path: Path):
        ...
        k = precedent.PrecedentKey(regime="X", session_phase="Y")
        g.record(k, precedent.PrecedentEntry(
            ts=datetime.now(UTC), action="TRADE", realized_r=0.5,
        ))
        p = tmp_path / "pg.json"
        g.save(p)
...
class TestNLQuery:
    ...
    def audit(self, tmp_path: Path):
        ...
        now = datetime.now(UTC)
        records = [
            {"request_id": "abc123", "verdict": "DENIED",
             "reason": "kill_blocks_all", "reason_code": "kill_blocks_all",
             "subsystem": "bot.mnq", "ts": now.isoformat(),
             "stress_composite": 0.6, "binding_constraint": "equity_dd"},
            {"request_id": "def456", "verdict": "APPROVED",
             "reason": "all gates green", "reason_code": "trade_ok",
             "subsystem": "bot.mnq", "ts": now.isoformat(),
...
class TestAlertsExplain:
    ...
    def test_narrative_injected_first(self):
        exp = alerts_explain.build_explanation(
            alert_code="fomc_imminent", severity="CRITICAL",
            contributions={}, raw_values={}, thresholds={},
...
class TestBudget:
    ...
    def test_pinned_never_downgrades(self):
        ...
        routed, note = t.routed_tier(
            ModelTier.OPUS, TaskCategory.RED_TEAM_SCORING, now=now,
        )
