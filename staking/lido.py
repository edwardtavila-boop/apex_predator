...
* **get_balance** — reads ``wstETH.balanceOf(wallet)`` on Ethereum mainnet via
  :mod:`apex_predator.staking.web3_client`. Falls back to in-memory
  ``_balance`` when ``rpc_url`` / ``wallet_address`` aren't provided OR web3
  isn't installed.
* **get_apy** — pulls live APY from :class:`apex_predator.staking.apy_tracker.ApyTracker`
...
* **stake/unstake** — construct structured contract-call payloads via
  :func:`apex_predator.staking.web3_client.build_contract_call`. Signing +
...
from apex_predator.staking.apy_tracker import ApyTracker, get_shared_tracker
