...
-----------------------
* `router`: an ``apex_predator.venues.router.SmartRouter`` instance (or anything
...
* `strategy_adapter`: optional
  :class:`apex_predator.strategies.engine_adapter.RouterAdapter`. When
...
from apex_predator.brain.jarvis_gate import (
    ask_jarvis,
    pick_llm_tier,
    record_gate_event,
)
from apex_predator.obs.decision_journal import Actor, DecisionJournal, Outcome
from apex_predator.venues.base import OrderRequest, OrderResult, OrderStatus, Side

if TYPE_CHECKING:
    from collections.abc import Callable

    from apex_predator.bots.retrospective_adapter import ActiveEntry
    ...
    from apex_predator.strategies.retrospective import RetrospectiveReport
    from apex_predator.strategies.retrospective_wiring import (
...
class MnqBot(BaseBot):
    ...
    def pick_model_tier(
        ...
        *,
        ...
        """
        if self._jarvis is None:
            from apex_predator.brain.model_policy import ModelTier as _ModelTier
    ...
    def _track_entry_from_signal(
        ...
        """
        from apex_predator.bots.retrospective_adapter import (
            ActiveEntry,
            compute_risk_usd,
    ...
    def record_fill(self, fill: Fill) -> RetrospectiveReport | None:
        ...
        from apex_predator.bots.retrospective_adapter import is_close_fill
    ...
    def _tick_retrospective(self, regime: RegimeType) -> None:
        ...
        if self._retrospective_manager is None:
            return
        from apex_predator.bots.retrospective_adapter import map_regime
        try:
