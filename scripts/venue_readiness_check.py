...
def check_venues() -> list[VenueStatus]:
    ...
    results.append(VenueStatus(
        "IBKR", bool(ibkr_files), ibkr_connector, ibkr_connector,
    ...
    results.append(VenueStatus(
        "Tastytrade", tt_connector, tt_connector, tt_connector,
    ...
    results.append(VenueStatus(
        "Tradovate", bool(tv_files), bool(tv_files), False,
        "DORMANT", str(tv_files[0]) if tv_files else "—",
    ))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        out = {
            "venues": [{"venue": v.venue, "connector_present": v.connector_present,
                        "sim_supported": v.sim_supported, "live_supported": v.live_supported,
                        "status": v.status, "path": v.path} for v in venues],
        ...
        print("-" * 80)
        for v in venues:
            print(f"{v.venue:<16} {v.status:<10} {'YES' if v.sim_supported else 'no':<6} {'YES' if v.live_supported else 'no':<6} {v.path}")
