"""
APEX PREDATOR  //  scripts.paper_run_harness
============================================
...
Usage:
    python -m apex_predator.scripts.paper_run_harness [--weeks 4] [--seed 11]
"""
...
def _ctx_builder(bar, hist):  # noqa: ANN001
    ...
    from apex_predator.core.data_pipeline import FundingRate
    now = bar.timestamp
...
def _write_report(
    ...
    ts_lines: list[str] = []
    ts_lines.append(f"APEX PREDATOR -- Paper Run Tearsheet  ({label or 'default'}, {bar_mode})")
    ts_lines.append("=" * 96)
    ts_lines.append(
...
def main() -> int:
    p = argparse.ArgumentParser(description="Apex Predator paper-run harness")
    ...
    print("APEX PREDATOR -- Paper-Run Harness")
    print("=" * 96)
