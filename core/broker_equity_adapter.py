...
ledger in ``docs/red_team_d2_d3_review.md``.
  ...
  * M2 -- KillVerdict synthesis on out-of-tolerance. Lands when:
    ...
    ``test_run_apex_live`` grows an integration test asserting
...
class RouterBackedBrokerEquityAdapter:
    ...
    :meth:`apex_predator.venues.router.SmartRouter.choose_venue` on every
    ...
    * ``name`` is a stable identifier (default ``"router-active-futures"``)
    ...
    ----------
    router:
        A :class:`apex_predator.venues.router.SmartRouter` instance. The
    ...
    def __init__(
        ...
        *,
        ...
        from apex_predator.venues.router import SmartRouter as _SmartRouter
