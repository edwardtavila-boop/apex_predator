...
from apex_predator.brain.jarvis_context import (
    ActionSuggestion,
    EquitySnapshot,
    JarvisContext,
...
def _ok_regime(regime: str = "TRENDING_UP",
               flipped: bool = False) -> RegimeSnapshot:
    return RegimeSnapshot(
        regime=regime, confidence=0.8, flipped_recently=flipped,
    )
...
def test_equity_rejects_negative_equity() -> None:
    with pytest.raises(ValidationError):
        EquitySnapshot(
            account_equity=-1.0, daily_pnl=0.0,
...
def test_equity_rejects_dd_over_100pct() -> None:
    with pytest.raises(ValidationError):
        EquitySnapshot(
            account_equity=1000.0, daily_pnl=0.0,
            daily_drawdown_pct=1.5, open_positions=0, open_risk_r=0.0,
        )
...
def test_kill_when_kill_switch_active() -> None:
    s = suggest_action(
        _ok_macro(), _ok_equity(), _ok_regime(), _ok_journal(kill=True),
    )
...
def test_kill_when_dd_over_5pct() -> None:
    s = suggest_action(
        _ok_macro(), _ok_equity(dd_pct=0.06), _ok_regime(), _ok_journal(),
    )
    assert s.action == ActionSuggestion.KILL
...
def test_stand_aside_when_dd_3pct() -> None:
    s = suggest_action(
        _ok_macro(), _ok_equity(dd_pct=0.035), _ok_regime(), _ok_journal(),
    )
    assert s.action == ActionSuggestion.STAND_ASIDE
...
def test_reduce_when_open_risk_over_3r() -> None:
    s = suggest_action(
        _ok_macro(), _ok_equity(open_risk_r=3.5, positions=3),
        _ok_regime(), _ok_journal(),
    )
    assert s.action == ActionSuggestion.REDUCE
...
def test_reduce_when_crisis_regime() -> None:
    s = suggest_action(
        _ok_macro(), _ok_equity(), _ok_regime(regime="CRISIS"), _ok_journal(),
    )
    assert s.action == ActionSuggestion.REDUCE
...
def test_kill_beats_stand_aside() -> None:
    ...
    macro = MacroSnapshot(
        next_event_label="CPI", hours_until_next_event=0.1,
    )
    s = suggest_action(
        macro, _ok_equity(dd_pct=0.06), _ok_regime(), _ok_journal(),
    )
    assert s.action == ActionSuggestion.KILL
...
def test_stand_aside_beats_reduce() -> None:
    ...
    macro = MacroSnapshot(
        next_event_label="FOMC", hours_until_next_event=0.25,
    )
    s = suggest_action(
        macro, _ok_equity(dd_pct=0.025), _ok_regime(), _ok_journal(),
    )
    assert s.action == ActionSuggestion.STAND_ASIDE
...
def test_no_event_label_means_no_stand_aside() -> None:
    macro = MacroSnapshot(
        next_event_label=None, hours_until_next_event=0.1,
    )
    s = suggest_action(macro, _ok_equity(), _ok_regime(), _ok_journal())
    assert s.action == ActionSuggestion.TRADE
...
def test_build_snapshot_preserves_ts() -> None:
    ...
    ctx = build_snapshot(
        macro=_ok_macro(), equity=_ok_equity(),
        regime=_ok_regime(), journal=_ok_journal(), ts=ts,
    )
    assert ctx.ts == ts
...
def test_build_snapshot_attaches_notes() -> None:
    ctx = build_snapshot(
        macro=_ok_macro(), equity=_ok_equity(),
        regime=_ok_regime(), journal=_ok_journal(),
...
def test_builder_snapshot_pulls_from_providers() -> None:
    ...
    b = JarvisContextBuilder(
        macro_provider=_MP(), equity_provider=ep,
        regime_provider=_RP(), journal_provider=_JP(),
    )
    ctx1 = b.snapshot()
    ctx2 = b.snapshot()
    assert ep.calls == 2
...
def test_builder_injectable_clock() -> None:
    ...
    b = JarvisContextBuilder(
        macro_provider=_MP(), equity_provider=_EP(),
        regime_provider=_RP(), journal_provider=_JP(),
        clock=lambda: fixed,
    )
    ctx = b.snapshot()
