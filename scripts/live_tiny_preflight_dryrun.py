"""
APEX PREDATOR  //  scripts.live_tiny_preflight_dryrun
======================================================
...
Usage
-----
    python -m apex_predator.scripts.live_tiny_preflight_dryrun
    python -m apex_predator.scripts.live_tiny_preflight_dryrun --inject-fail tradovate_creds
    python -m apex_predator.scripts.live_tiny_preflight_dryrun --simulate-red
...
-------------
    ...
    pytest_green           — smoke-test subset of apex_predator pytest passes
...
def _gate_runtime_wired() -> Gate:
    """The live runtime trio must exist + be importable:
       ...
       - scripts/run_apex_live.py     (the loop)
    ...
    paths = {
        ...
        "run_apex_live.py":       ROOT / "scripts" / "run_apex_live.py",
    }
    ...
    for mod_name in (
        "apex_predator.core.kill_switch_runtime",
        "apex_predator.obs.alert_dispatcher",
        "apex_predator.scripts.run_apex_live",
    ):
        try:
    ...
    if import_errors:
        ...
        g.status = "PASS"
        g.detail = "kill_switch + alerts + run_apex_live all importable"
...
def _gate_pytest_subset() -> Gate:
    ...
    target = "apex_predator/tests/test_paper_run_harness.py"
    try:
        out = subprocess.run(
...
def main() -> int:
    ...
    print("APEX PREDATOR -- Live-Tiny Preflight Dry Run")
    print("=" * 80)
    ...
    lines: list[str] = []
    lines.append("APEX PREDATOR -- Preflight Dryrun Log")
    lines.append("=" * 80)
    for g in gates:
        flag = "REQ" if g.required else "opt"
