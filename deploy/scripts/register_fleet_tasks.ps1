# APEX PREDATOR // register_fleet_tasks.ps1
# ==========================================
...
#     C:\apex_predator\deploy\scripts\register_fleet_tasks.ps1 `
#     -BtcAutoSubmit -McpRoot C:\apex_predator
#
# Idempotent: existing tasks with the same name are updated in place.

[CmdletBinding()]
param(
    [string]$McpRoot = "C:\apex_predator",
    [string]$PythonExe = "C:\apex_predator\.venv\Scripts\python.exe",
...
Register-ApexTask `
    ...
    -Arguments "-m apex_predator.scripts.btc_broker_fleet --start" `
    -EnvVars $btcEnv
...
if ($RegisterMnqSupervisor) {
    ...
    Register-ApexTask `
        ...
        -WorkingDir $McpRoot `
        -Executable $PythonExe `
        -Arguments "-m apex_predator.scripts.mnq_live_supervisor --bars ""$MnqBarsPath"""
}
