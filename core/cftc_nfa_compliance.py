"""
APEX PREDATOR  //  core.cftc_nfa_compliance
===========================================
...
class PreTradeContext(BaseModel):
    ...
    # Apex eval context
    apex_account_id: str | None = None
    other_apex_account_ids: list[str] = Field(default_factory=list)
...
def _check_apex_one_account(ctx: PreTradeContext) -> ComplianceViolation | None:
    ...
    # this by requiring that apex_account_id, when present, is the only
    # Apex account currently engaged in the same symbol.
    if (
        ctx.apex_account_id is not None
        and len(ctx.other_apex_account_ids) > 0
    ):
        return ComplianceViolation(
            ...
            message=(
                ...
                f"{[ctx.apex_account_id, *ctx.other_apex_account_ids]}. "
...
def _check_apex_no_cross_hedge(ctx: PreTradeContext) -> ComplianceViolation | None:
    if ctx.apex_account_id is not None and ctx.has_opposing_apex_position:
        return ComplianceViolation(
