"""
APEX PREDATOR  //  scripts.authorize_tradovate
==============================================
...
into ``apex_predator.venues.tradovate.TradovateVenue.authenticate()``.
...
Usage:
    python -m apex_predator.scripts.authorize_tradovate             # demo
    python -m apex_predator.scripts.authorize_tradovate --live      # live URL
    python -m apex_predator.scripts.authorize_tradovate --json      # machine-readable
...
# Allow `python -m apex_predator.scripts.authorize_tradovate` from either parent.
_ROOT = Path(__file__).resolve().parents[1]
_PARENT = _ROOT.parent
if str(_PARENT) not in sys.path:
    sys.path.insert(0, str(_PARENT))

from apex_predator.core.secrets import (  # noqa: E402
    SECRETS,
    TRADOVATE_APP_ID,
...
from apex_predator.venues.tradovate import (  # noqa: E402
    TRADOVATE_DEMO,
    TRADOVATE_LIVE,
    TradovateVenue,
)
...
async def _run(demo: bool) -> tuple[int, AuthReport]:
    ...
    if not report.has_all_creds:
        ...
        report.reason = (
            ...
            f"{','.join(missing)} -- populate via keyring or apex_predator/.env "
            f"and rerun"
        )
...
def _print_human(report: AuthReport) -> None:
    print()
    print("APEX PREDATOR -- Tradovate Authorization")
    print("=" * 66)
