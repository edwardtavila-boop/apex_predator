...
class TestWindowConstruction:
    ...
    def test_rolling_produces_windows(self) -> None:
        bars = BarReplay.synthetic_bars(
            n=4 * 24 * 15, drift=0.0005, vol=0.004, seed=4,
            start=datetime(2025, 1, 1, tzinfo=UTC), interval_minutes=15,
        )  # ~15 days of 15m bars
        eng = WalkForwardEngine()
        wins = eng._build_windows(bars, WalkForwardConfig(
            window_days=5, step_days=2, anchored=False, oos_fraction=0.4,
        ))
        assert len(wins) >= 2
    ...
    def test_anchored_keeps_is_start_fixed(self) -> None:
        bars = BarReplay.synthetic_bars(
            n=4 * 24 * 15, drift=0.0005, vol=0.004, seed=5,
            start=datetime(2025, 1, 1, tzinfo=UTC), interval_minutes=15,
        )
        eng = WalkForwardEngine()
        wins = eng._build_windows(bars, WalkForwardConfig(
            window_days=5, step_days=2, anchored=True, oos_fraction=0.4,
        ))
        if len(wins) >= 2:
...
class TestEndToEnd:
    ...
    def test_run_yields_result(self) -> None:
        bars = BarReplay.synthetic_bars(
            n=4 * 24 * 20, drift=0.0015, vol=0.004, seed=7,
            start=datetime(2025, 1, 1, tzinfo=UTC), interval_minutes=15,
        )
        eng = WalkForwardEngine()
        res = eng.run(
