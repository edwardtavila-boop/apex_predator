...
class TestRunTask:
    ...
    def test_unknown_task_exits_nonzero(self, tmp_path):
        from apex_predator.deploy.scripts.run_task import main
        rc = main([
    ...
    def test_doctrine_review_writes_report(self, tmp_path):
        ...
        assert "APEX PREDATOR DOCTRINE" in out
...
class TestSmokeCheck:
    ...
    def test_imports_pass(self):
        from apex_predator.deploy.scripts.smoke_check import check_imports
        ok, _ = check_imports()
        assert ok

    def test_dirs_check_creates_dirs(self):
        from apex_predator.deploy.scripts.smoke_check import check_dirs
        ok, _ = check_dirs()
        assert ok

    def test_dispatch_check_runs(self):
        from apex_predator.deploy.scripts.smoke_check import check_dispatch
        ok, msg = check_dispatch()
        assert ok, msg

    def test_task_handlers_check_passes(self):
        from apex_predator.deploy.scripts.smoke_check import check_task_handlers
    ...
    def test_smoke_with_skip_systemd(self, tmp_path, monkeypatch):
        ...
        from apex_predator.deploy.scripts.smoke_check import main
...
class TestDeployArtifacts:
    ...
    def test_systemd_units_all_present(self):
        for unit in ("jarvis-live.service", "avengers-fleet.service",
    ...
    def test_crontab_has_all_tasks(self):
        ...
        for line in text.splitlines():
            if "run_task" in line and not line.strip().startswith("#"):
                assert "apex-predator:avengers" in line
