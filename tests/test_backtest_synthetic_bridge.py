...
from apex_predator.backtest.synthetic_bridge import (
    SCENARIO_REGIME_MAP,
    bars_from_returns,
    scenario_to_regime,
    synthetic_scenario_bars,
)
from apex_predator.brain.regime import RegimeType
...
class TestBarsFromReturns:
    def test_empty_returns_empty_bars(self):
        bars = bars_from_returns(
            [], start_price=100.0, regime=RegimeType.HIGH_VOL, seed=42
        )
        assert bars == []
    ...
    def test_returns_materialize_into_n_bars(self):
        ...
        bars = bars_from_returns(
            returns, start_price=100.0, regime=RegimeType.HIGH_VOL, seed=42
        )
        assert len(bars) == 4
    ...
    def test_ohlc_invariants(self):
        ...
        bars = bars_from_returns(
            returns, start_price=1000.0, regime=RegimeType.CRISIS, seed=7
        )
        for b in bars:
    ...
    def test_determinism_same_seed_same_bars(self):
        returns = [0.01, -0.02, 0.005]
        b1 = bars_from_returns(
            returns, start_price=100.0, regime=RegimeType.HIGH_VOL, seed=42
        )
        b2 = bars_from_returns(
            returns, start_price=100.0, regime=RegimeType.HIGH_VOL, seed=42
        )
    ...
    def test_rejects_zero_start_price(self):
        with pytest.raises(ValueError):
            bars_from_returns(
                [0.01], start_price=0.0, regime=RegimeType.HIGH_VOL, seed=0
            )
...
class TestSyntheticScenarioBars:
    ...
    def test_bars_reflect_crash_magnitude(self):
        ...
        worst_drop = min(
            (b.close / bars[i - 1].close - 1.0) for i, b in enumerate(bars) if i > 0
        )
        # Flash-crash scenario imposes at least a -10% bar
        assert worst_drop < -0.05

    def test_slow_grind_has_negative_total_return(self):
        _, spec = synthetic_scenario_bars(
            "2008_slow_grind", n_bars=250, start_price=100.0, seed=2008
        )
