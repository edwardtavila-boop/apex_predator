...
def test_slippage_tracker_resolves_buy_side(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    st.record_expected(order_id="o1", symbol="MNQ", side="buy",
                       expected_price=21450.0, ts=1000.0)
...
def test_slippage_tracker_resolves_sell_side(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    st.record_expected(order_id="o2", symbol="MNQ", side="sell",
                       expected_price=21500.0, ts=2000.0)
...
def test_macro_calendar_detects_fomc_window() -> None:
    ...
    fomc = MacroEvent(MacroEventKind.FOMC, datetime(2026, 6, 17, 18, 0, tzinfo=UTC),
                      "FOMC Jun", "high")
...
def test_macro_calendar_presser_uses_wider_window() -> None:
    ...
    presser = MacroEvent(MacroEventKind.FOMC_PRESSER, datetime(2026, 6, 17, 18, 30, tzinfo=UTC),
                         "Powell Jun", "high")
...
def test_sentiment_snapshot_roundtrip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    snap = ss.SentimentSnapshot(
        symbol="MNQ", ts=datetime.now(UTC),
        composite=0.5, news_score=0.6, social_score=0.4,
        volume_z=1.5, n_news_articles=20, n_social_mentions=500,
    )
...
def test_sentiment_confluence_modifier_aligns() -> None:
    ...
    snap = SentimentSnapshot(
        symbol="MNQ", ts=datetime.now(UTC),
        composite=0.6, news_score=0.6, social_score=0.6,
        volume_z=1.0, n_news_articles=10, n_social_mentions=100,
    )
...
def test_onchain_snapshot_roundtrip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    snap = oc.OnchainSnapshot(
        symbol="BTCUSDT", ts=datetime.now(UTC),
        funding_rate_8h=-0.001, open_interest_usd=15_000_000_000.0,
        net_exchange_flow_usd=-50_000_000.0, whale_tx_count_24h=80,
        btc_dominance_pct=53.0, realized_vol_30d=0.55,
    )
...
def test_onchain_confluence_signals_negative_funding_bullish_for_long() -> None:
    ...
    snap = OnchainSnapshot(
        symbol="BTCUSDT", ts=datetime.now(UTC),
        funding_rate_8h=-0.0015, open_interest_usd=None,
        net_exchange_flow_usd=None, whale_tx_count_24h=None,
        btc_dominance_pct=None, realized_vol_30d=None,
    )
...
def test_cumulative_delta_alignment_long_with_rising_cd() -> None:
    ...
    bars = [
        FlowBar(ts_ms=i, open=100, high=101, low=99, close=100.5,
                buy_volume=100 + i * 5, sell_volume=80)
        for i in range(15)
    ]
    series = compute_flow_series(bars)
...
def test_basis_regime_label_classifies_correctly() -> None:
    ...
    backwardation = BasisSnapshot(
        symbol="MBT", ts=datetime.now(UTC),
        spot_price=95000.0, futures_price=94800.0,
        basis_pct=-0.0021, days_to_expiry=30, annualized_basis=-0.025,
    )
    assert regime_label(backwardation) == "BACKWARDATION"

    normal = BasisSnapshot(
        symbol="MBT", ts=datetime.now(UTC),
        spot_price=95000.0, futures_price=95200.0,
        basis_pct=0.0021, days_to_expiry=30, annualized_basis=0.025,
    )
    ...
    steep = BasisSnapshot(
        symbol="MBT", ts=datetime.now(UTC),
        spot_price=95000.0, futures_price=96500.0,
        basis_pct=0.0158, days_to_expiry=30, annualized_basis=0.20,
    )
