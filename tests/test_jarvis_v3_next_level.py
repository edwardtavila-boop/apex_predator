...
class TestDebate:
    ...
    def test_bull_argues_approve_when_stress_low(self):
        arg = debate.bull_argue(
            stress=0.2, sizing_mult=1.0, regime="NEUTRAL", suggestion="TRADE",
        )
    ...
    def test_bear_argues_deny_in_crisis(self):
        arg = debate.bear_argue(
            stress=0.8, sizing_mult=0.3, regime="CRISIS",
            suggestion="REDUCE", dd_pct=0.03,
        )
        assert arg.vote == "DENY"

    def test_full_debate_aggregates_four(self):
        v = debate.full_debate(
            stress=0.3, sizing_mult=0.9, regime="NEUTRAL",
            regime_confidence=0.8, suggestion="TRADE",
            precedent_n=20, precedent_win_rate=0.65, precedent_mean_r=0.8,
    ...
    def test_historian_no_precedent(self):
        arg = debate.historian_argue(
            precedent_n=0, precedent_win_rate=None, precedent_mean_r=None,
        )
...
class TestShadow:
    ...
    def test_open_close_long_winner(self):
        ...
        t = shadow.shadow_from_denied_request(
            request_id="r1", subsystem="bot.mnq", symbol="MNQ",
            side="LONG", entry_px=20000.0, stop_px=19900.0, target_px=20200.0,
        )
        ledger.add(t)
        changed = ledger.tick(
    ...
    def test_short_loser(self):
        ledger = shadow.ShadowLedger()
        t = shadow.shadow_from_denied_request(
            request_id="r2", subsystem="bot.btc", symbol="BTC",
            side="SHORT", entry_px=60000.0, stop_px=60500.0, target_px=59000.0,
        )
        ledger.add(t)
        ledger.tick(
    ...
    def test_regret_summary(self):
        ...
        for i, r in enumerate([2.0, 1.5, 2.0, -1.0]):
            ledger.add(shadow.ShadowTrade(
                id=f"t{i}", opened_at=datetime.now(UTC),
                subsystem="x", symbol="S", side="LONG",
                entry_px=100, stop_px=99, target_px=102,
                r_distance=1.0, realized_r=r, status=shadow.ShadowStatus.CLOSED,
                closed_at=datetime.now(UTC), closed_px=100,
            ))
        s = ledger.regret()
    ...
    def test_expiration(self):
        ...
        now = datetime.now(UTC)
        ledger.add(shadow.ShadowTrade(
            id="t1", opened_at=now - timedelta(hours=5),
            subsystem="x", symbol="S", side="LONG",
            entry_px=100, stop_px=99, target_px=102, r_distance=1.0,
        ))
...
class TestVectorPrecedent:
    ...
    def test_exact_match_is_highest(self):
        ...
        now = datetime.now(UTC)
        store.record(
            entry_id="e1", ts=now, regime="CRISIS", session_phase="OPEN_DRIVE",
            event_category="fomc", action="DENY",
            realized_r=-1.0, outcome_correct=1,
        )
        store.record(
            entry_id="e2", ts=now, regime="RISK_ON", session_phase="MORNING",
            event_category="none", action="TRADE",
            realized_r=+1.0, outcome_correct=1,
        )
        hits = store.search(
            regime="CRISIS", session_phase="OPEN_DRIVE", event_category="fomc",
        )
    ...
    def test_synthesize_mean_r(self):
        ...
        for i, r in enumerate([1.0, 0.8, 1.2, 0.9]):
            store.record(
                entry_id=f"e{i}", ts=now,
                regime="RISK_ON", session_phase="MORNING",
                action="TRADE", realized_r=r, outcome_correct=1,
            )
...
class TestStrategySynthesis:
    ...
    def test_candidate_emitted_on_strong_bucket(self):
        ...
        for _i in range(25):
            g.record(k, PrecedentEntry(
                ts=datetime.now(UTC), action="TRADE",
                realized_r=1.0, outcome_correct=1,
            ))
...
class TestVoice:
    ...
    def test_briefing_under_budget(self, tmp_path):
        ...
        b = hub.build_briefing(
            regime="NEUTRAL", session_phase="MORNING", stress=0.2,
            open_risk_r=1.0, daily_dd_pct=0.0,
        )
    ...
    def test_inbound_dispatches(self, tmp_path):
        ...
        reply = hub.handle_inbound(voice.InboundMessage(
            ts=datetime.now(UTC), channel=voice.Channel.TELEGRAM,
...
class TestDigitalTwin:
    ...
    def test_divergence_detected(self):
        cmp_ = digital_twin.TwinComparator()
        now = datetime.now(UTC)
        cmp_.ingest(digital_twin.TwinSignal(
            ts=now, source="PROD", signal_id="s1", subsystem="bot.mnq",
            verdict="DENY", size_mult=0.0,
        ))
        cmp_.ingest(digital_twin.TwinSignal(
            ts=now, source="TWIN", signal_id="s1", subsystem="bot.mnq",
            verdict="APPROVE", size_mult=0.8,
        ))
    ...
    def test_high_divergence_avoid(self):
        ...
        for i in range(10):
            cmp_.ingest(digital_twin.TwinSignal(
                ts=now, source="PROD", signal_id=f"s{i}",
                subsystem="bot.mnq", verdict="DENY", size_mult=0.0,
            ))
            tw_verdict = "APPROVE" if i < 4 else "DENY"
            cmp_.ingest(digital_twin.TwinSignal(
                ts=now, source="TWIN", signal_id=f"s{i}",
                subsystem="bot.mnq", verdict=tw_verdict, size_mult=0.0,
            ))
        v = cmp_.verdict()
...
class TestAutoPR:
    ...
    def _ticket(self, impact="medium", title="Fix: whatever"):
        return KaizenTicket(
            ...
            title=title, rationale="test rationale",
            impact=impact, status=KaizenStatus.OPEN,
            opened_at=datetime.now(UTC),
        )

    def test_small_scope(self):
        plan = autopr.build_plan(self._ticket(
            impact="small", title="Fix typo in README"))
...
class TestCausal:
    ...
    def test_ate_on_synthetic_data(self):
        ...
        res = causal.counterfactual_denied(
            dag, confounders=["stress_composite", "regime_code"],
        )
    ...
    def test_no_overlap_returns_note(self):
        dag = causal.CausalDAG()
        dag.add_observation({
            "verdict_denied": 1.0, "stress_composite": 0.9,
            "regime_code": 1.0, "realized_r": 0.0,
        })
...
class TestSelfPlay:
    ...
    def test_default_policy_denies_risk_off(self):
        event = self_play.MarketEvent(
            ...
            regime_hint="RISK_OFF", truth_regime="RISK_OFF",
            stress_pushed=0.8, realized_r_if_trade=-1.5,
        )
