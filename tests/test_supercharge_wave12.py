...
def test_intelligence_passes_through_denied_verdict(tmp_path: Path) -> None:
    ...
    admin.request_approval.return_value = _stub_action_response(
        "DENIED", reason_code="kill_switch_armed",
    )
    intel = JarvisIntelligence(
        admin=admin, memory=None,
        cfg=IntelligenceConfig(enable_intelligence=False),
...
def test_intelligence_causal_veto_only_downgrades_when_enabled(tmp_path: Path) -> None:
    ...
    for _ in range(8):
        mem.record_episode(
            signal_id="bad", regime="bullish_low_vol", session="rth",
    ...
    intel_no_veto = JarvisIntelligence(
        ...
        cfg=IntelligenceConfig(
            enable_intelligence=True, causal_veto_can_downgrade=False,
        ),
...
def test_close_trade_records_episode(tmp_path: Path) -> None:
    ...
    rec = close_trade(
        signal_id="s1", realized_r=1.5,
        regime="bullish_low_vol", session="rth", stress=0.3,
        direction="long", narrative="winner",
        action_taken="approve_full", bot_id="testbot",
        memory=mem,
...
def test_close_trade_updates_filter_bandit(tmp_path: Path) -> None:
    ...
    rec = close_trade(
        signal_id="s2", realized_r=2.0,
        regime="neutral", session="rth", stress=0.5,
        direction="long",
        action_taken="approve_full",
        filter_bandit=fb, filter_arm_used="ema_filter",
        trade_log_path=tmp_path / "trades.jsonl",
    )
...
def test_replay_trade_closes_into_memory(tmp_path: Path) -> None:
    ...
    for i in range(3):
        close_trade(
            signal_id=f"s{i}", realized_r=1.0,
            regime="neutral", session="rth", stress=0.5,
            direction="long",
            action_taken="approve_full",
            trade_log_path=log_path,
...
def test_recent_verdicts_returns_zero_when_no_log(tmp_path: Path) -> None:
    ...
    rep = recent_verdicts(
        n_hours=24, log_path=tmp_path / "missing.jsonl",
    )
...
def test_recent_verdicts_aggregates_log(tmp_path: Path) -> None:
    ...
    rows = [
        {"ts": now, "final_verdict": "APPROVED", "subsystem": "MNQ_BOT",
         "confidence": 0.8, "rag_cautions": [], "firm_board_consensus": 0.8},
        {"ts": now, "final_verdict": "DEFERRED", "subsystem": "BTC_BOT",
         "confidence": 0.5, "rag_cautions": ["lost analog"],
...
def test_memory_regime_stats_aggregates(tmp_path: Path) -> None:
    ...
    for r in [-1.0, 0.5]:
        mem.record_episode(
            signal_id=f"b{r}", regime="bearish_low_vol", session="rth",
            stress=0.5, direction="short", realized_r=r,
        )
...
def test_top_analog_episodes_returns_sorted_results(tmp_path: Path) -> None:
    ...
    mem.record_episode(
        signal_id="match", regime="neutral", session="rth", stress=0.5,
        direction="long", realized_r=1.0,
        narrative="liquidity sweep reclaim with order block",
    )
    mem.record_episode(
        signal_id="other", regime="neutral", session="rth", stress=0.5,
        direction="long", realized_r=-0.3,
...
def test_disagreement_hotspots_groups_by_pair(tmp_path: Path) -> None:
    ...
    rows = [
        {"ts": now, "subsystem": "MNQ", "action": "ORDER",
         "firm_board_consensus": 0.3},
        {"ts": now, "subsystem": "MNQ", "action": "ORDER",
         "firm_board_consensus": 0.4},
        {"ts": now, "subsystem": "MNQ", "action": "ORDER",
         "firm_board_consensus": 0.35},
        {"ts": now, "subsystem": "BTC", "action": "ORDER",
         "firm_board_consensus": 0.9},
        {"ts": now, "subsystem": "BTC", "action": "ORDER",
         "firm_board_consensus": 0.95},
        {"ts": now, "subsystem": "BTC", "action": "ORDER",
         "firm_board_consensus": 0.85},
    ]
    log.write_text(
        "\n".join(json.dumps(r) for r in rows) + "\n",
        encoding="utf-8",
    ...
    spots = disagreement_hotspots(
        n_hours=24, min_count=3, log_path=log,
    )
