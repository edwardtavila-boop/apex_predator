...
Verifies:
  ...
  7. Crontab has the apex-predator:avengers tag (if install step 7 ran)
...
def check_dirs() -> tuple[bool, str]:
    state_dir = Path.home() / ".local" / "state" / "apex_predator"
    log_dir = Path.home() / ".local" / "log" / "apex_predator"
...
def check_crontab() -> tuple[bool, str]:
    ...
    if "apex-predator:avengers" not in out:
        return False, "no apex-predator:avengers tag in crontab. Re-run install step 7."
    count = out.count("apex-predator:avengers")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("APEX PREDATOR  //  smoke check")
    print("=" * 60)
    failures = 0
