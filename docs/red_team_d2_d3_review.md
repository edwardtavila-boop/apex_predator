...
|----|----------|-------|-------|
| B3 | BLOCKER  | tier-A aggregate equity invariant undocumented (sum-of-bot-state.equity vs single broker net-liq comparison may be apples-to-oranges) | **CLOSED v0.1.69** — `core/eta_account_invariant.py` codifies the invariant; ApexRuntime fires `tier_a_invariant_violation` (advisory WARN) on transition into a violation; default oversize 1.5× / undersize 0.0× catches the canonical config-bug case; +25 tests |
