...
from apex_predator.scripts.sample_size_calc import (
    Z95,
    _compute_row,
    _pool_portfolio,
)
...
def test_integration_run_on_canonical(tmp_path: Path):
    ...
    ``apex_predator`` package (e.g. some CI layouts put the tests in a
    sibling of the package root) -- the subprocess invocation below
    relies on ``-m apex_predator.scripts.sample_size_calc`` resolving,
