...
`$global:APEX_STATE   = "`$env:LOCALAPPDATA\apex_predator\state"
`$global:APEX_LOGS    = "`$env:LOCALAPPDATA\apex_predator\logs"
...
function apex-dashboard { Get-Content "`$global:APEX_STATE\dashboard_payload.json" | ConvertFrom-Json | ConvertTo-Json -Depth 6 }
...
"@
    ...
    OK "aliases: apex-status, apex-heartbeat, apex-dashboard, apex-tasks, apex-logs, apex-restart, apex-test, apex-task, apex-health"
}
