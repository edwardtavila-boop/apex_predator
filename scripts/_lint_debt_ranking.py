...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--by-rule", action="store_true",
                   help="rank by rule code instead of by file")
    p.add_argument("--max-yellow", type=int, default=100,
                   help="if total violations > N -> RED (default 100)")
    ...
    total = len(findings)
    print(
        f"lint-debt-ranking: {total} total violations across "
        f"{len(by_file)} file(s), {len(by_rule)} rule(s)",
    )
    print()
    ...
    level = (
        "GREEN" if total == 0
        else "RED" if total > args.max_yellow
        else "YELLOW"
    )
