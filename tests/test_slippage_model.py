...
class TestTiers:
    ...
    def test_normal_uses_half_spread(self) -> None:
        ...
        bps = m.estimate("ETHUSDT", "BUY", qty=0.01, price=3500.0,
                         vol_pct=0.01, urgency="NORMAL")
        assert abs(bps - 0.25) < 0.1

    def test_aggressive_greater_than_normal(self) -> None:
        m = SlippageModel()
        n = m.estimate("SOLUSDT", "BUY", qty=100.0, price=200.0,
                       vol_pct=1.0, urgency="NORMAL")
        a = m.estimate("SOLUSDT", "BUY", qty=100.0, price=200.0,
                       vol_pct=1.0, urgency="AGGRESSIVE")
        assert a > n
    ...
    def test_sqrt_qty_impact_scaling(self) -> None:
        m = SlippageModel()
        low = m.estimate("ETHUSDT", "BUY", qty=100.0, price=3500.0,
                         vol_pct=1.0, urgency="NORMAL")
        high = m.estimate("ETHUSDT", "BUY", qty=10_000.0, price=3500.0,
                          vol_pct=1.0, urgency="NORMAL")
...
class TestDefaults:
    ...
    def test_mnq_has_narrow_spread(self) -> None:
        m = SlippageModel()
        bps = m.estimate("MNQ", "BUY", qty=1.0, price=20500.0,
                         vol_pct=0.5, urgency="NORMAL")
        assert bps < 1.0
