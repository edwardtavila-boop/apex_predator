...
def test_walk_forward_aggregates_metrics() -> None:
    ...
    rep = run_walk_forward(
        ...
        cfg=WalkForwardConfig(
            ...
            target_sharpe=0.5, max_dd_r=20.0,
...
def test_walk_forward_passes_when_distribution_strong() -> None:
    ...
    rep = run_walk_forward(
        ...
        cfg=WalkForwardConfig(
            train_size=100, test_size=50, step=50,
            target_sharpe=0.5, max_dd_r=10.0,
            min_aggregate_trades=50, psr_threshold=0.5,
        ),
    )
...
def test_pre_live_gate_fails_on_negative_distribution(tmp_path: Path) -> None:
    ...
    decision = evaluate_for_live(
        candidate_id="bad_v1", recent_r_multiples=rs,
        replay_lift=0.0, regression_pass_rate=1.0,
...
def test_pre_live_gate_passes_on_strong_distribution(tmp_path: Path) -> None:
    ...
    decision = evaluate_for_live(
        candidate_id="strong_v1", recent_r_multiples=rs,
        replay_lift=0.2, regression_pass_rate=1.0,
        cfg=PreLiveGateConfig(
            min_sharpe=0.8, min_psr=0.5, max_drawdown_r=20.0,
            min_trades=50, min_is_oos_ratio=0.3,
        ),
        decisions_log_path=tmp_path / "decisions.jsonl",
    )
...
def test_pre_live_gate_persists_decision(tmp_path: Path) -> None:
    ...
    evaluate_for_live(
        candidate_id="persist", recent_r_multiples=[0.5] * 200,
...
def test_route_signal_zero_split_always_control() -> None:
    ...
    for i in range(20):
        assert route_signal(
            signal_id=f"s{i}", traffic_split=0.0,
        ) == "control"
...
def test_ab_manager_caps_traffic_split(tmp_path: Path) -> None:
    ...
    exp = mgr.register_experiment(
        experiment_id="e1", traffic_split=0.50,
...
def test_ab_manager_kills_on_single_loss(tmp_path: Path) -> None:
    ...
    mgr.register_experiment(
        experiment_id="e1", traffic_split=0.10,
        bounds=AbBounds(single_loss_kill_r=2.0),
    )
    # Single -3R loss -> kill
    exp = mgr.record_outcome(
        experiment_id="e1", variant="treatment", realized_r=-3.0,
    )
...
def test_ab_manager_records_stats(tmp_path: Path) -> None:
    ...
    mgr.record_outcome(
        experiment_id="e2", variant="treatment", realized_r=1.0,
    )
    mgr.record_outcome(
        experiment_id="e2", variant="control", realized_r=0.5,
    )
    exp = mgr.get("e2")
    assert exp is not None
...
def test_ab_manager_can_declare_winner_with_enough_data(tmp_path: Path) -> None:
    ...
    mgr = AbManager(state_path=tmp_path / "ab.json")
    mgr.register_experiment(
        experiment_id="e3", traffic_split=0.10,
        bounds=AbBounds(min_sample_size=10, significance_alpha=0.05,
                        single_loss_kill_r=10.0,
                        cumulative_dd_kill_r=20.0),
    )
    # Treatment consistently better
    rng = random.Random(7)
    for _ in range(40):
        mgr.record_outcome(
            experiment_id="e3", variant="treatment",
            realized_r=rng.gauss(1.5, 0.5),
        )
        mgr.record_outcome(
            experiment_id="e3", variant="control",
...
def test_ab_manager_cannot_declare_with_too_few_samples(tmp_path: Path) -> None:
    ...
    mgr = AbManager(state_path=tmp_path / "ab.json")
    mgr.register_experiment(
        experiment_id="e4", traffic_split=0.10,
        bounds=AbBounds(min_sample_size=30, single_loss_kill_r=10.0),
    )
    for _ in range(5):
        mgr.record_outcome(
            experiment_id="e4", variant="treatment", realized_r=1.0,
        )
        mgr.record_outcome(
            experiment_id="e4", variant="control", realized_r=0.0,
        )
...
def test_regression_suite_add_and_evaluate_pass_case(tmp_path: Path) -> None:
    ...
    suite.add_case(
        case_id="winner1", kind=CaseKind.PASS_CASE,
        signal_id="s1", proposal_payload={"sage_score": 0.6},
        realized_r=2.0, rationale="clean winner",
    )
...
def test_regression_suite_detects_regression_on_pass_case(tmp_path: Path) -> None:
    ...
    suite = RegressionSuite(cases_path=tmp_path / "cases.json")
    suite.add_case(
        case_id="winner2", kind=CaseKind.PASS_CASE,
        signal_id="s2", proposal_payload={"x": 1},
        realized_r=2.0,
    )
...
def test_regression_suite_detects_regression_on_fail_case(tmp_path: Path) -> None:
    ...
    suite = RegressionSuite(cases_path=tmp_path / "cases.json")
    suite.add_case(
        case_id="loser1", kind=CaseKind.FAIL_CASE,
        signal_id="s3", proposal_payload={"x": 1},
        realized_r=-2.5, rationale="catastrophic loss",
    )
...
def test_regression_suite_persists_across_instances(tmp_path: Path) -> None:
    ...
    s1.add_case(
        case_id="persist", kind=CaseKind.PASS_CASE,
        signal_id="x", proposal_payload={"y": 2},
        realized_r=1.5,
    )
...
def test_regression_suite_remove_case(tmp_path: Path) -> None:
    ...
    suite = RegressionSuite(cases_path=tmp_path / "cases.json")
    suite.add_case(
        case_id="rm1", kind=CaseKind.PASS_CASE,
        signal_id="s", proposal_payload={"x": 1}, realized_r=1.0,
    )
