"""
APEX PREDATOR — Runtime Orchestrator
====================================
Boots the 6-bot fleet, wires data feeds, starts funnel + staking monitors.

Usage:
    python -m apex_predator.main --mode paper --bots mnq,eth_perp
    python -m apex_predator.main --mode live --bots all
    python -m apex_predator.main --mode replay --date 2026-03-15
...
logger = logging.getLogger("apex_predator")
...
class ApexPredator:
    ...
    async def init_funnel(self) -> None:
        ...
        # from apex_predator.funnel.equity_monitor import EquityMonitor
        # from apex_predator.staking.allocator import Allocator
    ...
    async def init_brain(self) -> None:
        ...
        # from apex_predator.brain.regime import classify_regime
        # from apex_predator.brain.rl_agent import RLAgent
        # from apex_predator.brain.multi_agent import MultiAgentOrchestrator
        logger.warning("Brain not yet wired — scaffold phase")

    async def start(self) -> None:
        """Boot sequence."""
        logger.info("APEX PREDATOR starting in mode=%s unpause=%s", self.mode, self.unpause)
    ...
    async def shutdown(self) -> None:
        ...
        logger.warning("APEX PREDATOR shutting down — flattening positions")
        self.running = False
        # TODO: flatten all positions, persist state, close feeds


def main() -> int:
    parser = argparse.ArgumentParser(description="APEX PREDATOR runtime orchestrator")
    ...
    try:
        ...
        logger.exception("APEX PREDATOR crashed: %s", e)
        return 1
