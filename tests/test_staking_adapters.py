...
Covers:
* :func:`apex_predator.staking.web3_client.read_balance` fallback paths
  (no RPC, no wallet, web3 uninstalled, RPC exception).
* :func:`apex_predator.staking.web3_client.build_contract_call` payload shape.
* :class:`apex_predator.staking.apy_tracker.ApyTracker` cache hit/miss,
...
from apex_predator.staking import apy_tracker as apy_module
from apex_predator.staking import web3_client
from apex_predator.staking.apy_tracker import ApyTracker
...
from apex_predator.staking.web3_client import build_contract_call, read_balance
...
async def test_lido_on_chain_balance_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from apex_predator.staking import lido as lido_module
...
async def test_jito_spl_balance_path(monkeypatch: pytest.MonkeyPatch) -> None:
    from apex_predator.staking import jito as jito_module
...
async def test_flare_stake_with_delegation(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="apex_predator.staking.flare"):
...
async def test_ethena_unstake_logs_cooldown(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="apex_predator.staking.ethena"):
