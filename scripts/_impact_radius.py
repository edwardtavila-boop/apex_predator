...
-----
    ...
    python scripts/_impact_radius.py apex_predator.strategies.engine_adapter.EngineAdapter
...
def _enclosing_ctx(node: ast.AST, lineno: int) -> str:
    ...
    for child in ast.walk(node):
        if isinstance(child, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            ...
            if start <= lineno <= end:
                depth = (
                    child.col_offset if hasattr(child, "col_offset") else 0
                )
                if depth >= best_depth:
                    best = child.name
                    best_depth = depth
...
def _scan_file(path: Path, target: str) -> list[Hit]:
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Name) and node.id == short_target:
            ...
            hits.append(Hit(
                path=path, line=ln, kind=kind,
                ctx=_enclosing_ctx(tree, ln), snippet=_snippet(ln),
            ))
        elif isinstance(node, ast.Attribute) and node.attr == short_target:
            hits.append(Hit(
                path=path, line=ln, kind="attr",
                ctx=_enclosing_ctx(tree, ln), snippet=_snippet(ln),
            ))
            ...
            for alias in node.names:
                if alias.name == short_target:
                    hits.append(Hit(
                        path=path, line=ln, kind="import",
                        ctx="<module>", snippet=_snippet(ln),
                    ))
    return hits
...
def _scan_repo(target: str) -> Index:
    ...
    for path in ROOT.rglob("*.py"):
        if any(p in {".git", ".venv", "__pycache__", ".pytest_cache",
                     ".cache", "node_modules"} for p in path.parts):
            continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--tests-only", action="store_true",
                   help="only show hits inside tests/")
    ...
    print(f"impact-radius: '{args.target}' -- "
          f"{total} hit(s) in {len(idx.by_file)} file(s) "
          f"({src_hits} source, {test_hits} test)")
    print(body)
    print(f"\nTest coverage of hits: {test_hits}/{total} "
          f"({(test_hits / total * 100):.0f}%)")
    return 0
