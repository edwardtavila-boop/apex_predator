...
* Compute the dotted path ``apex_predator.<package>.<sub>.<name>``
* Search every other .py in the repo for either:
    ``from apex_predator.<package>.<sub>.<name> ...``
    ``import apex_predator.<package>.<sub>.<name>``
    ``from apex_predator.<package>.<sub> import <name>``
...
def _dotted(rel: Path) -> str:
    ...
    if parts[-1].endswith(".py"):
        parts[-1] = parts[-1][:-3]
    return "apex_predator." + ".".join(parts)
