#!/usr/bin/env bash
# ============================================================================
# APEX PREDATOR // install_vps.sh
# ----------------------------------------------------------------------------
# Idempotent installer for the Apex Predator stack (JARVIS + Avengers + bots).
#
# Safe to re-run.  Every step checks before it writes.
#
# Usage (run on the VPS as the operator user, NOT root):
#   curl -fsSL https://raw.githubusercontent.com/<you>/apex_predator/main/deploy/install_vps.sh \
#     | bash -s -- --repo-url https://github.com/<you>/apex_predator.git --branch main
#
# Or cloned locally:
...
#   2. Clones / pulls the repo into $INSTALL_DIR (default ~/apex_predator)
...
# ----------------------------------------------------------------------------
INSTALL_DIR="${INSTALL_DIR:-$HOME/apex_predator}"
REPO_URL="${REPO_URL:-}"
BRANCH="${BRANCH:-main}"
...
mkdir -p "$UNIT_DIR"
mkdir -p "$HOME/.local/state/apex_predator"
mkdir -p "$HOME/.local/log/apex_predator"

for unit in jarvis-live.service avengers-fleet.service apex-dashboard.service; do
...
if [[ -f "$CRON_SRC" ]]; then
  ...
  MERGED="$(mktemp)"
  {
    printf '%s\n' "$EXISTING" | grep -v '# apex-predator:avengers' || true
    printf '\n'
    cat "$TMP_CRON"
  } > "$MERGED"
  if [[ "$DRY_RUN" == "1" ]]; then
...
cat <<EOF
  ...
  4. Start the services (in order):
     ...
     systemctl --user start apex-dashboard
